...
export default function About({ data }) {
  ...
  const cards = [
    {
      ...
      icon: (
        <svg width="15" height="19" viewBox="0 0 18 22" fill="none">
    ...
    },
    {
      ...
      border: "border-brand-charcoal/20",
      icon: (
        <svg width="16" height="19" viewBox="0 0 19 22" fill="none">
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute bottom-4 left-2 select-none pointer-events-none z-20"
      >
        <span
          className="font-black leading-none tracking-tighter uppercase"
          style={{ fontSize: "clamp(80px, 12vw, 160px)", color: "rgba(255,255,255,0.04)" }}
        >
          ABOUT
        </span>
      </div>
      ...
      <div className="relative z-20 h-full w-full flex items-center">
        <div className="flex items-center flex-1 px-8 sm:px-12 lg:px-16 xl:px-24">
        <div className="w-full max-w-135">
          ...
          <motion.div
            ...
            className="w-10 h-0.75 bg-brand-yellow origin-left mb-5 rounded-full"
          />
          ...
          {badgeText && (
            <motion.p
              ...
              className="text-xs font-bold tracking-[3px] uppercase text-white/60 mb-3"
            >
              {badgeText}
            </motion.p>
          )}
          ...
          >
            <p className="text-[15px] text-white/70 leading-relaxed">{description1}</p>
            <p className="text-[15px] text-white/70 leading-relaxed">{description2}</p>
          </motion.div>

          {/* Feature cards — 2×2 */}
          <StaggerContainer className="grid grid-cols-2 gap-2.5 mb-7">
            {cards.map((card, i) => (
              <StaggerItem
                key={i}
                className="group flex items-start gap-3 p-3.5 rounded-2xl bg-white/10 border border-white/15 hover:border-brand-yellow hover:bg-white/15 hover:shadow-[0_4px_20px_rgba(247,227,38,0.15)] transition-all duration-300 cursor-default"
              >
                <div
                  className={`w-9 h-9 flex items-center justify-center ${card.iconBg} rounded-xl shrink-0 transition-transform duration-300 group-hover:scale-110`}
                >
                  {card.icon}
                </div>
                <div className="min-w-0">
                  <h3 className="font-bold text-white text-[13px] leading-tight">{card.title}</h3>
                  <p className="text-[11px] text-white/55 mt-0.5 leading-relaxed">{card.desc}</p>
                </div>
              </StaggerItem>
            ))}
          </StaggerContainer>
