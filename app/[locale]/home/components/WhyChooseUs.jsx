...
export default function WhyChooseUs({ data }) {
  ...
  const MobileLayout = () => (
    <div id="why-choose-us" style={{ background: "#0f0f0f" }}>
      <div className="px-8 pt-16 pb-10">
        <span className="text-xs font-bold tracking-[4px] uppercase font-mono mb-6 block" style={{ color: "rgba(255,255,255,0.35)" }}>
          {t("why_choose_us") || "WHY CHOOSE US"}
        </span>
        <div className="w-10 h-0.75 bg-brand-yellow rounded-full mb-6" />
        <h2 className="font-black leading-tight tracking-tight text-white" style={{ fontSize: "clamp(1.8rem, 8vw, 2.5rem)" }}>
          {headerTitle}
        </h2>
      </div>
      {panels.map((panel, i) => (
        ...
        >
          ...
          <span className="text-xs font-mono font-bold mb-5 block" style={{ color: "#F7E326" }}>
            {String(i + 1).padStart(2, "0")} — {t("why_choose_us") || "WHY CHOOSE US"}
          </span>
          <div className="w-8 h-0.75 bg-brand-yellow/40 rounded-full mb-6" />
      ...
      <div ref={sectionRef} id="why-choose-us" className="hidden md:block" style={{ height: "500vh" }}>
        <section style={{ position: "sticky", top: 0, height: "100vh", overflow: "hidden" }}>
          {panels.map((panel, i) => (
            <div
              key={i}
              className={`why-panel${i > 0 ? " why-panel-offscreen" : ""}`}
              style={{ position: "absolute", inset: 0, background: panel.bg }}
            >
              {/* Background number */}
              <span
                aria-hidden="true"
              ...
              <div className="absolute top-10 left-10 sm:left-14 lg:left-20 xl:left-28 flex items-center gap-4">
                <span className="text-xs font-mono font-bold" style={{ color: "rgba(255,255,255,0.35)" }}>
                  {t("why_choose_us") || "WHY CHOOSE US"} — {String(i + 1).padStart(2, "0")} / {String(panels.length).padStart(2, "0")}
                </span>
              </div>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center px-10 sm:px-20 lg:px-32 xl:px-48 text-center">
                <span className="text-xs font-mono font-bold mb-8 block" style={{ color: "#F7E326" }}>
                  {String(i + 1).padStart(2, "0")}
                </span>

                <div className="w-10 h-0.75 bg-brand-yellow rounded-full mb-8" />
