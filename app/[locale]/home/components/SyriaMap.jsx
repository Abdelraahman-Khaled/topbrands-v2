...
/**
 * Animated map of Syria for the hero section.
 * Renders the national outline, a glowing dot for every governorate capital,
 * the "constellation" lines linking them, and a comet that travels the route,
 * visiting every governorate in sequence.
 *
 * Coordinates live in the SVG user space (viewBox 0 0 1027 1024). The outline
 * path is a potrace trace flipped via the wrapping <g> transform; the dots were
 * projected from each capital's real lat/lng into that same space.
 */

// Syria national outline (potrace). Drawn inside a flipping <g> transform.
const SYRIA_PATH =
...
// Governorate capitals, in the order the comet visits them (projected lat/lng).
const GOVERNORATES = [
  { name: "Latakia", x: 36, y: 374 },
  { name: "Idlib", x: 161, y: 301 },
  { name: "Aleppo", x: 236, y: 251 },
  { name: "Al-Hasakah", x: 774, y: 197 },
  { name: "Raqqa", x: 515, y: 297 },
  { name: "Deir ez-Zor", x: 683, y: 409 },
  { name: "Al-Bukamal", x: 799, y: 571 },
  { name: "Hama", x: 180, y: 447 },
  { name: "Homs", x: 173, y: 520 },
  { name: "Tartus", x: 50, y: 491 },
  { name: "Damascus", x: 110, y: 742 },
  { name: "Quneitra", x: 41, y: 812 },
  { name: "Daraa", x: 83, y: 904 },
  { name: "As-Suwayda", x: 152, y: 888 },
];

const YELLOW = "#f7e326";

export default function SyriaMap({ className = "" }) {
  const reduceMotion = useReducedMotion();

  // Closed loop for the constellation polyline and the traveling marker.
  const loop = [...GOVERNORATES, GOVERNORATES[0]];
  const cometPath =
    `M${loop[0].x},${loop[0].y} ` +
    loop.slice(1).map((g) => `L${g.x},${g.y}`).join(" ") +
    " Z";
  const TOTAL = 18; // seconds for one full lap
  ...
  return (
    ...
    >
      <defs>
        <filter id="syria-glow" x="-60%" y="-60%" width="220%" height="220%">
      ...
      {/* Constellation lines linking the governorates */}
      <motion.polyline
        points={loop.map((g) => `${g.x},${g.y}`).join(" ")}
        fill="none"
        stroke={YELLOW}
        strokeWidth="1.5"
        strokeOpacity="0.25"
      ...
      {/* Traveling marker — native SVG motion so it loops reliably */}
      {!reduceMotion && (
        <>
          <circle r="9" fill={YELLOW} fillOpacity="0.25" filter="url(#syria-glow)">
            <animateMotion dur={`${TOTAL}s`} repeatCount="indefinite" path={cometPath} calcMode="linear" />
          </circle>
          <circle r="5" fill="#fffbe0" filter="url(#syria-glow)">
            <animateMotion dur={`${TOTAL}s`} repeatCount="indefinite" path={cometPath} calcMode="linear" />
          </circle>
        </>
      )}
