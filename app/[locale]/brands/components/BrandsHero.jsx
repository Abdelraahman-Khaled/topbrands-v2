...
export default function BrandsHero({ data }) {
  ...
  return (
    <section className="relative min-h-[80vh] flex items-end overflow-hidden" style={{ background: "#0f0f0f" }}>

      {imageUrl && (
        <div className="absolute inset-0 z-0">
          <img src={imageUrl} alt={title || "Brands"} className="w-full h-full object-cover" />
          <div
            className="absolute inset-0"
            style={{ background: "linear-gradient(to right, rgba(0,0,0,0.9) 35%, rgba(0,0,0,0.4) 100%)" }}
          />
        </div>
      )}
